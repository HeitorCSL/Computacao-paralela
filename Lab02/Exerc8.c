...
int functransposta(int y,int x,int matriz[y][x]) // função que cria transposta
{
  ...
  for (int j=0; j<x; j++) //x colunas que se tornarao y linhas
  {
    for (int i=0; i<y; i++) // y linhas que se tornao x colunas
    {
...
int main (){
  ...
  functransposta(taml,tamc,matriz); //função que chama a transposta
}